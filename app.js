...
form.addEventListener("submit", function(event) {
    ...
    let numCoursesInput = document.getElementById("numCourses");
    let numCourses = parseInt(numCoursesInput.value);
    for (let i = 0; i < numCourses; i++) {
        let inputDiv = document.createElement("div");
        inputDiv.className = "input_pair_div";
        ...
        inputDiv.appendChild(courseInput);
        inputDiv.appendChild(gradeInput);
        inputContainer.appendChild(inputDiv);
    }
    let calculateBtn = document.createElement("button");
    calculateBtn.className = "btn";
    calculateBtn.innerText = "Calculate CGPA";
    calculateBtn.addEventListener("click", calculateCGPA);
    inputContainer.appendChild(calculateBtn);
});
function calculateCGPA(){
    let inputPairs = document.querySelectorAll(".input_pair_div");
    let totalCredits = 0;
    let weightedSum = 0;
    inputPairs.forEach(pair => {
        let courseCode = pair.querySelector(".course-input").value.trim();
        let grade = pair.querySelector(".grade-input").value.trim().toUpperCase();
        let course = course_details.courseDetails.find(c => c.code === courseCode);
        if (course && grades_details[grade] !== undefined) {
            let credits = parseInt(course.credits);
            let gradeValue = grades_details[grade];
            totalCredits += credits;
            weightedSum += credits * gradeValue;
        }
    });
    let cgpa = weightedSum / totalCredits;
    let resultContainer = document.querySelector(".resultContainer");
    resultContainer.innerHTML = `Your CGPA is: ${cgpa.toFixed(2)}`;
}